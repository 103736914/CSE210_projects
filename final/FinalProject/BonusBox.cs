public class BonusBox : Box
{
    ...
    public override string Menu()
    {
        while (true)
        {
            ...
            if (userInput == 1)
            {
                ...
                Console.Clear();
                Console.WriteLine($"{string.Concat(Enumerable.Repeat("> ", userInput + 1)) + Name} (BonusBox)");
                foreach (Element element in Elements)
                {
                    element.Display(userInput - 1);
                }
    ...
    {
        Console.WriteLine($"{string.Concat(Enumerable.Repeat("> ", depth + 1))}");
        if (Elements.Count == 0)
        {
        Console.WriteLine($"{Name} (BonusBox; empty)");
        }
        else if (Elements.Count == 1)
        {
        Console.WriteLine($"{Name} (BonusBox; contains 1 element)");
        }
        else
        {
        Console.WriteLine($"{Name} (BonusBox; contains {Elements.Count} elements)");
        }
    }
}
